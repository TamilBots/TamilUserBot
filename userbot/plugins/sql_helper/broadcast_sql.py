...
class UserBroadcast(BASE):
    ...
    def __repr__(self):
        return "<User Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, UserBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


UserBroadcast.__table__.create(checkfirst=True)

USERBROADCAST_INSERTION_LOCK = threading.RLock()
...
def del_keyword_broadcastlist(keywoard):
    with CATBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(UserBroadcast.keywoard)
            .filter(UserBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(UserBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(UserBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(UserBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(UserBroadcast).all()
        for x in all_groups:
