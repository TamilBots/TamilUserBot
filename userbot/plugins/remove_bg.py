...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit("Please wait Analysing this pic...")
        try:
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "TAMIL_UB_BG.png"
        ...
        await event.edit(
            "Removed dat annoying Backgroup in {} seconds, powered by @TamilUserBot™".format(
                ms
            )
        )
    else:
        await event.edit(
            "ReMove.BG API returned Errors. Please report to @TamilSupport\n`{}".format(
